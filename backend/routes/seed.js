...
// Seed studio designs
router.post('/studio-designs', async (req, res) => {
  try {
    ...
    res.json({ 
      success: true, 
      message: `Successfully seeded ${results.length} studio designs!`,
      designs: results
    });
  } catch (error) {
    console.error('❌ Error seeding designs:', error);
    res.status(500).json({ error: 'Failed to seed designs', details: error.message });
  }
});
